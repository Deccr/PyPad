...
# Mission failed, we'll get him next time.
    # I couldn't get an icon to work
...
def about():
    ...
    txt = "PYPad by DeccrXD\nAll code on github\nhttps://github.com/deccr/\n"
...
def web():
    ...
    webbrowser.open('https://github.com/deccr/')


root = Tk()

root.title("PyPad by Deccr")
...
modmenu.add_command(label = "Clear Highlighted", command = clear)

modmenu.add_command(label = "Clear Entire Document", command = clearall)
...
formatmenu.add_cascade(label="Font Type", command = font)

formatmenu.add_separator()

formatmenu.add_radiobutton(label='Normal',command=normal)

formatmenu.add_radiobutton(label='B O L D',command=bold)
